// Quake3 Raytracing Engine - C99 with code-golfing
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <GL/gl.h>
...
GLuint mkPrg(const char *v, const char *f) {
    ...
    GLuint V = mkShd(GL_VERTEX_SHADER, v);
    GLuint F = mkShd(GL_FRAGMENT_SHADER, f);
    glAttachShader(P, V);
    glAttachShader(P, F);
    glLinkProgram(P);
    glDeleteShader(V);
...
int main() {
    ...
    GLFWwindow *w = glfwCreateWindow(W, H, "Quake3 Raytracer", NULL, NULL);
    if (!w) {
        glfwTerminate();
        return -1;
    }
    ...
    printf("Quake3 Raytracing Engine\n");
    printf("Press ESC to exit\n");
    printf("Press P to save screenshot\n\n");
